    ...
    if (access == false) {
      return bot
        .createEmbed(
          "error",
          "",
          `Error! You are not the owner or the admin on this guild.`,
          [],
          `${message.guild.name}`,
          bot
        )
    ...
    if (setting == undefined) {
      return bot
        .createEmbed(
          "error",
          "",
          `Error! You forgot to include a config setting to change.`,
          [],
          `${message.guild.name}`,
          bot
        )
    ...
    switch (setting) {
        ...
        if (config.welcomerenabled) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Welcomer is already enabled!`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (!config.welcomerenabled) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Welcomer is already disabled!`,
              [],
              `${message.guild.name}`,
              bot
            )
      ...
      case "channel":
        ...
        if (targetchannel == undefined) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! You didn't mentionn a channel!`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (targetchannel.id == config.welcomerchannel) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! That channel is already set as the welcomer channel!`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        bot
          .createEmbed(
            "success",
            "",
            `The welcomer channel has been set to **${targetchannel.name}**`,
            [],
            `${message.guild.name}`,
            bot
          )
        ...
        if (setmessage.length < 1) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! You forgot to include a message!`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (setmessage.length > 254) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your message is too long! It needs to be less than **254** characters.`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (setmessage == config.welcomermessage) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your message is the same as the current one!`,
              [],
              `${message.guild.name}`,
              bot
            )
      ...
      case "test":
        ...
        if (config.welcomerenabled == false) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your configuration didn't work. This was because you haven't enabled welcomer yet! You can do so by doing **sb!config-welcomer enable**`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (config.welcomerchannel == 0) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your configuration didn't work. This was because you haven't set a channel for your welcome messages. You can do so by doing **sb!config-welcomer channel <#CHANNEL>**`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (testingchannel == undefined) {
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your configuration didn't work. This is beacuse the channel you have set no longer exists. Please set a new channel by doing **sb!config-welcomer channel <#CHANNEL>**`,
              [],
              `${message.guild.name}`,
              bot
            )
        ...
        if (
          botmember
            .permissionsIn(
              message.guild.channels.get("" + config.welcomerchannel + "")
            )
          ...
          return bot
            .createEmbed(
              "error",
              "",
              `Error! Your configuration didn't work. This is because the bot is unable to send messages in the configured channel you have set.`,
              [],
              `${message.guild.name}`,
              bot
            )
      ...
      default:
        bot
          .createEmbed(
            "error",
            "",
            `Error! There isn't a welcomer config setting called **${setting}**`,
            [],
            `${message.guild.name}`,
            bot
          )
