module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    var targetuser = await bot.users.fetch(args[0].replace(/[^0-9]/g, ''), {
      force: true
    });
    ...
    if (message.guild.bans.fetch(targetuser, {
        force: true
      })) {
    ...
    message.guild.bans.create(targetuser, {
        reason: `Banned by ${message.author.tag}${isReason ? ` with reason ${reason}.` : `.`}`
      })
      .then(() => {
        bot.createEmbed("success", "", `Sucessfully banned ${targetuser}${isReason ? ` for \`${reason}\`.` : `.`}`, [], `${message.guild.name}`, message)
          .then((embed) => message.reply(embed))
          .catch((error) => bot.log.post("error", error))
