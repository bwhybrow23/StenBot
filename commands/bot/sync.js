module.exports = {
    ...
    function syncServers(g) {
     ...
     bot.guilds.forEach(g => {
      ...
      if (guildexists == false) {
       ...
       fs.writeFileSync(`./data/servers/server-${g.id}/serverstats.json`, JSON.stringify(stats, null, 4), (err) => {
        if (err) return;
       });
       ...
       fs.writeFileSync(`./data/servers/server-${g.id}/serverconfig.json`, JSON.stringify(defaultContent, null, 4), (err) => {
        if (err) return;
       });
   
       //levelling system
       let levelDefault = {};
       fs.writeFileSync(`./data/servers/server-${g.id}/levelling.json`, JSON.stringify(levelDefault, null, 4), (err) => {
        if (err) return;
       });
   
       //Check if files were generated successfully.
       var locations = ["/", "/users", "/tempbans", "/serverstats.json", "/serverconfig.json", "/levelling.json"];
       let gsuccess = true;
    ...
    message.channel.send({
     embed: {
      ...
      description: `**${amountsynced}** servers have been synced in ${attempt} attempts.`
     }
    });
