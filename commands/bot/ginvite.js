    ...
    if (!guild) {
      bot
        .createEmbed(
          "error",
          "",
          `Error! The bot isn't in a guild with that ID.`,
          [],
          `${message.guild.name}`,
          bot
        )
    ...
    if (!invitechannels) {
      bot
        .createEmbed(
          "error",
          "",
          `Error! I don't have permission to create an invite in that guild.`,
          [],
          `${message.guild.name}`,
          bot
        )
    ...
    invitechannels
      ...
      .then((invite) =>
        message.channel.send(
          `Found Invite: **https://discord.gg/${invite.code}**, Server name: **${guild.name}**`
        )
      );
  },
};
