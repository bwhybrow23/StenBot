class Queuing {
  ...
  addToQueue(object, firstElementExecute = true, notExecButSkip = false) {
    return new Promise((resolve, reject) => {
      ...
      if (object.afterEvent) {
        if (!object.afterEvent.emitter || !object.afterEvent.event)
          return reject("Missing emitter or event options in afterEvent");
      }
      if (typeof object.value == "function") {
        object.args = object.args || [];
        if (!Array.isArray(object.args))
          return reject("Args option needs to be an array");
        funcType = true;
      }
      ...
      if (notExecButSkip && this.queue.length == 1)
        return this._queueNextUp(newObject);
      if (
        !firstElementExecute &&
        object.afterEvent != null &&
        this.queue.length == 1
      )
        return this._queueNextUp(newObject, true);
      if (firstElementExecute && this.queue.length == 1 && this.paused == false)
        this._executeQueue(resolve, reject);
    });
  }
  ...
  _queueNextUp(queueF, skip = false) {
    ...
    if (skip) {
      let emitter =
        typeof queueF.afterEvent.emitter == "function"
          ? queueF.afterEvent.emitter()
          : queueF.afterEvent.emitter;
      if (!emitter) return;
      return emitter.once(
        queueF.afterEvent.event,
        waitingToPassSkip.bind(this, queueF)
      );
    }
    if (queueF.eventNeed) {
      let emitter =
      ...
      setTimeout(() => {
        // Adding a timeout of 5 ms so if the user pause the queue direclty after one finish it will be paused
        if (this.paused) return;
        this.queue.shift();
        this._executeQueue();
      }, 5);
