		...
		if (config.logginglevel == "high") {
			if (efunctions.checkChannel(config.loggingchannel, bot) == true) {
				...
				if (oldRole.name !== newRole.name) {
					lchannel.send({embed: {color: bot.settings.yellow, description: `**Role Name Changed**\n**Before:** ${oldRole.name}\n**After:** ${newRole.name}\n**ID:** ${newRole.id}`, footer: {text: 'Role Name Changed'}, timestamp: new Date()}})
