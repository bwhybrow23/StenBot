...
export default {
  ...
  async execute(bot, interaction) {
    ...
    if (interaction.customId === 'userModal') {
      ...
      if (!rObj) {
        interaction.reply({ content: 'A user with this ID cannot be found. Please refer to [this guide](http://bit.ly/getdiscordid) for more information.', flags: 64 });
        return;
      }
      ...
      bot.createEmbed('success', 'User Report Sent', 'Your user report has been sent and will be reviewed by the Staff Team of StenBot. Thank you for your help.', [], `${interaction.user.tag}`, interaction)
        .then(embed => interaction.reply({ embeds: embed, flags: 64 }))
        .catch(error => bot.log.post('error', error));
      ...
      if (!gObj) {
        interaction.reply({ content: 'A server with this ID cannot be found. Please refer to [this guide](http://bit.ly/getdiscordid) for more information.', flags: 64 });
        return;
      }
